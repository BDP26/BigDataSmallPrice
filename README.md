...
Daten- und ML-Pipelines mit Apache Airflow und stellt Prognosen über eine
FastAPI sowie Web-Dashboards bereit.
...
- Prognose der lokalen Winterthurer Netto-Netzlast aus Bruttolast und
  PV-Einspeisung.
- Berechnung eines geschätzten Gesamttarifs in Rp./kWh aus Energie-,
  Netzpreis- und Tarifkomponenten.
- Automatisierte Datenbeschaffung für ENTSO-E, Open-Meteo, EKZ, CKW,
  Groupe E, BAFU und Stadtwerk Winterthur.
- Feature Engineering für Zeitreihenmodelle mit Lags, rollierenden
  Kennzahlen, Kalenderfeatures, zyklischen Features und Wetterfeatures.
- Modelltraining mit Naive-Baselines, linearen Modellen, XGBoost, LSTM und
  Transformer-Varianten.
- User-Dashboard für Tarifprognosen und Admin-Dashboard für Datenbankstatus,
  Backfills, Trainingsjobs, Rate-Limits, Feature-Status und Modellmetriken.
...
| Projektdatenbank | TimescaleDB/PostgreSQL, `infra/db/init.sql` |
| Airflow-Metadatenbank | PostgreSQL |
| Backend | FastAPI, `src/api/main.py` |
| Frontend | Statische HTML/JS-Dashboards, `src/frontend/` |
...
| Quelle | Zweck |
| --- | --- |
| ENTSO-E Transparency Platform | Day-Ahead-Preise, tatsächliche Last, Lastprognosen, Erzeugung und Grenzflüsse |
| Open-Meteo | Wetterdaten für Winterthur sowie deutsche Wind- und Solar-Proxies |
...
fällt die Prognose auf einen Energy-only-Modus mit Standard-Netzpreis zurück.
...
| Dienst | URL |
| --- | --- |
| User-Dashboard über Nginx | http://localhost:8002 |
| User-Dashboard über API | http://localhost:8001/dashboard |
...
Port `8001` erreichbar. Das Nginx-Frontend läuft auf Port `8002` und proxyt
`/api/`, `/docs` und `/openapi.json` an die API.
...
| `bdsp_feature_daily` | täglich 07:00 | Erstellt Energy- und Load-Features und schreibt Parquet-Exporte |
...
Die DAGs sind beim Erstellen standardmäßig pausiert. Sie können in der
...
| `GET /api/db-explorer/rows/{table}` | Paginiertes Anzeigen von Tabellenzeilen |
| `GET /api/timeseries/{table}` | Zeitreihendaten für Dashboard-Charts |
...
| `GET /api/rate-limits/history` | Historie der API-Aufrufe |
...
| `GET /api/training/tasks/{dag_run_id}` | Task-Status eines Trainings-Runs |
...
|   `-- Dockerfile
|-- abschlussbericht/          # Abschlussbericht als LaTeX/PDF
...
|   `-- utils/                 # Hilfsskripte zur API-/Provider-Inspektion
|-- docker-compose.yml
|-- pyproject.toml
|-- requirements.txt
`-- README.md
...
- `airflow/dags/etl_pipeline_dag.py`: tägliche Datenbeschaffung.
- `airflow/dags/feature_pipeline_dag.py`: Export von Energy- und Load-Features.
- `airflow/dags/training_dag.py`: On-demand-Training der Modelle.
- `airflow/dags/backfill_dag.py`: historischer Backfill mit Rate-Limit-Schutz.
...
- `src/modelling/train.py`: Training klassischer und sequenzieller Modelle.
...
- `ekz_tariffs_hourly`, `ckw_tariffs_hourly`, `groupe_e_tariffs_hourly`
- `bafu_hydro`
...
- **Docker meldet ein GPU-/NVIDIA-Problem:** Das Training kann CPU-basiert
  laufen. Ohne NVIDIA Container Toolkit muss die GPU-Reservation im
  Airflow-Service von `docker-compose.yml` entfernt oder auskommentiert werden.
...
- **Admin-Dashboard unter `localhost:8002/admin` ist nicht erreichbar:** Das
  Admin-Dashboard wird vom API-Service unter `http://localhost:8001/`
  ausgeliefert. Port `8002` ist primär das Nginx-User-Dashboard.

## Dokumentation

- `docs/Proposal.md`: Projektproposal.
- `docs/req.md`: Anforderungen, Architektur und Teststrategie.
- `abschlussbericht/abschlussbericht.pdf`: Abschlussbericht.
