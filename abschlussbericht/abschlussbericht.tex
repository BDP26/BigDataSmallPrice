...
\renewcommand{\shortauthors}{Ryan Bachmann, Miguel Dinis Silva, Gian Ruchti}
...
beim Preismodell liegen beide unter den gesetzten Qualit\"atszielen.
...
\fancyhead[R]{Ryan Bachmann, Miguel Dinis Silva, Gian Ruchti}
\fancyfoot[C]{\thepage}
...
Datenpunkte.
Die Energiepreisvorhersage basiert auf ENTSO-E-Day-Ahead-Preisen seit 2024.
...
\begin{figure}[t]
  ...
  \begin{axis}[
    name=axB,
    at={(axA.north east)}, anchor=north west, xshift=24pt,
    width=0.52\columnwidth,
    height=3.8cm,
...
Interessant: Das lineare Modell schneidet aktuel bei der Lastprognose einwenig
besser ab als XGBoost, ein Zeichen, dass der Lastgang stark von
linearen Gr\"ossen wie Tageszeit und Temperatur getrieben wird. 
Weil diese \"Aenderung kurz vor dem Ende dieser Arbeit eintrat, 
haben wir uns entschieden vorerst noch XGBoost als Prognosemodel 
zu nutzen, da es w\"ahrend der Entwicklungszeit stetig besser abschnitt als das lineare Modell.
...
\begin{figure}[htb]
  ...
  \begin{axis}[
    ...
    \addplot[name path=oben, draw=red, smooth] coordinates {
    ...
    \addlegendentry{95\,\%-Konfidenzgrenzen}
    %% Untere Grenze des 95-%-Bandes
    \addplot[name path=unten, draw=red, smooth, forget plot] coordinates {
...
\begin{itemize}
  ...
  \item \textbf{Datenl\"ucken:} In \"alteren ENTSO-E-Erzeugungs\-daten
        (vor~2024) fehlen einzelne Werte. Die Median-Auff\"ullung reicht
...
bei der Energiepreisvorhersage und liegt damit unter den angestrebten
