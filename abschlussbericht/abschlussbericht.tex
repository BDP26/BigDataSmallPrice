...
% ZHAW Big Data Seminar HS 2025/2026
...
\acmYear{2026}
\acmDOI{}
\acmConference[Big Data HS25/26]{ZHAW Big Data Seminar}{HS 2025/26}{Winterthur, Schweiz}
...
% ---- Abstract ----
\begin{abstract}
Seit 2025 sind Schweizer Stromversorger verpflichtet, dynamische Tarife anzubieten,
die sich alle 15~Minuten \"andern k\"onnen.
...
\ccsdesc[200]{Computer systems organization~Distributed architectures}

\keywords{dynamische Stromtarife, Zeitreihenprognose, XGBoost, LSTM,
          ETL-Pipeline, TimescaleDB, Apache Airflow, ENTSO-E}
...
Das revidierte Stromversorgungsgesetz verpflichtet Netzbetreiber seit
Januar~2025, ihren Kunden variable Tarife anzubieten~\cite{ElCom2023}.
...
\begin{enumerate}
  ...
  \item Zwei kalibrierte Vorhersagemodelle f\"ur Netz- und Energiepreis mit
        t\"aglichem Retraining und messbaren Genauigkeitszielen.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{2.9cm}p{2.3cm}p{0.8cm}p{1.0cm}@{}}
    \toprule
    Quelle & Inhalt & Granul. & Format \\
    \midrule
    ENTSO-E (A44/A65/\newline A75/A11/A01)
      & Preise, Last, Erzeug., Fl\"usse & 15\,min/1\,h & REST/XML \\[2pt]
    Open-Meteo (CH + DE)
      & Temp., Wind, Solar, Wolken & 1\,h & REST/JSON \\[2pt]
    OGD Winterthur \#1863
      & Bruttolastgang (ab 2013) & 15\,min & CSV/API \\[2pt]
    OGD Winterthur \#3122
      & PV-Einspeisung & 15\,min & CSV/API \\[2pt]
    EKZ-Tarif-API
      & Referenztarif (Netz+Energie) & 15\,min & REST/JSON \\[2pt]
    CKW-Tarif-API
      & Vergleichstarif Innerschweiz & 15\,min & REST/JSON \\[2pt]
    Groupe~E-API
      & Vergleichstarif Romandie & 15\,min & REST/JSON \\[2pt]
    BAFU Hydrologie
...
die automatisch in der richtigen Reihenfolge ausgef\"uhrt werden:

\begin{itemize}
  \item \textbf{ETL-Pipeline:} L\"adt t\"aglich ab 06:00~UTC Daten von allen
        acht Quellen in bis zu 14 parallelen Prozessen herunter,
  ...
  \item \textbf{Training-Pipeline:} Trainiert beide Modelle t\"aglich neu,
        bewertet sie auf einem Validierungsset und speichert das jeweils beste
        Modell.
...
Abbildung~\ref{fig:dag} zeigt die Abh\"angigkeits\-struktur des
t\"aglichen Haupt-DAGs.
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}[
    ...
    \node[proc]  (tr) at (5.90, 0.60) {model\\training};
    ...
    \node[font=\tiny, text=gray!70] at (3.0, -0.30)
      {t\"aglich 06:00~UTC, nach ENTSO-E-Freigabe};
  \end{tikzpicture}
  \caption{Vereinfachter Airflow-DAG: Die vier Fetch-Tasks laufen parallel;
           anschliessend werden Daten validiert, in TimescaleDB gespeichert,
           Features exportiert und beide Modelle neu trainiert.}
  \label{fig:dag}
\end{figure}
...
Die vorhergesagte Nettolast wird zun\"achst auf den historischen
Schwankungsbereich normiert (Minimum~6\,000~kWh,
Maximum~17\,500~kWh, entsprechend dem~5. und~95. Lastperzentil):
\begin{equation}
  \hat{L}_{\mathrm{norm}} = \frac{\hat{L} - 6\,000}{17\,500 - 6\,000}
  \;\in [0, 1]
\end{equation}
...
Die REST-Schnittstelle stellt \"uber 40~Endpunkte bereit, darunter die
7-Tage-Prognose mit Konfidenzb\"andern (15-Minuten-Cache), Modell\-metriken
...
Die vollautomatische Pipeline l\"auft t\"aglich um 06:00~UTC;
der gesamte Zyklus von Datenabholung \"uber Feature-Engineering bis
zum frisch trainierten Produktionsmodell dauert weniger als~35~Minuten.
Das 7-Tage-Prognose-Endpunkt antwortet typischerweise in unter~200~ms;
ein~15-Minuten-Cache im Frontend reduziert die Daten\-bank\-belastung
bei vielen gleichzeitigen Nutzern um rund~97\,\%, sodass
...
Jede Datenquelle durchl\"auft nach dem Download einen Validierungsschritt:
Bereichspr\"ufung (Lastgang $>0$, Preise $>-500$\,EUR/MWh), Vollst\"andig\-keits\-
pr\"ufung (maximal~5\,\% fehlende Werte pro Tag erlaubt) und Plausibilit\"ats\-
pr\"ufung gegen den gleitenden Median der letzten~7~Tage.
Datens\"atze, die diese Checks nicht bestehen, l\"osen einen Airflow-Alert aus
und werden nicht f\"ur das Training verwendet.
Dieser Schritt war entscheidend, da mehrere Quellen gelegentlich
inkonsistente oder verz\"ogerte Daten lieferten.

\textbf{Parallelisierung (Value \& Processing):}
Die ETL-Pipeline f\"uhrt bis zu~14~Fetch-Tasks gleichzeitig aus,
was die Gesamtlaufzeit gegen\"uber sequenzieller Verarbeitung
von \"uber~20~auf unter~8~Minuten reduziert.
...
XGBoost ist in beiden F\"allen die beste Variante und erreichet die
...
Der API-Endpunkt f\"ur die 7-Tage-Prognose (672~Datenpunkte mit Tarifformeln
und Unsicherheitsgrenzen) antwortet typischerweise in unter~200~ms.
Dank des 15-Minuten-Caches im Frontend bleibt die Antwortzeit f\"ur
99\,\% der Dashboard-Aufrufe unter~5~ms, selbst bei hoher Nutzerzahl.
...
\begin{itemize}
  ...
  \item \textbf{Datenl\"ucken:} Vor allem in \"alteren ENTSO-E-Erzeugungsdaten
        (vor~2016) gibt es fehlende Werte. Die Auff\"ullung mit dem Median
        gen\"ugt f\"ur den Normalbetrieb, k\"onnte aber bei Extremereignissen
        (Hitzewellen, Netzst\"orungen) besser gel\"ost werden.
...
Mit \emph{BigDataSmallPrice} haben wir ein vollst\"andiges, betriebsnahes
System zur Prognose dynamischer Stromtarife f\"ur Winterthur entwickelt.
Die Plattform liest t\"aglich Daten aus acht Quellen ein, trainiert beide
Vorhersagemodelle neu und stellt die Ergebnisse \"uber eine REST-Schnittstelle
sowie ein interaktives Dashboard bereit.
